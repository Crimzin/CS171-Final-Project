...
function loadDataJ() {
    d3.csv("data/Industry2012.csv", function(error, data) {
        ...
        g.on('mouseover', function(d) {
            ...
            tooltip.select('.Industry').html("<strong>" + d.data.Industry + "</strong> <span>");
            tooltip.select('.Firms').html("Firms: " + addCommas(d.data.Firms));
            tooltip.select('.percent').html("Proportion: " + percent + '%');
...
function updateVisualizationj() {
    ...
    d3.csv("data/" + criteriaChosen + yearChosen + ".csv", function(error, data) {
        ...
        g.on('mouseover', function(d) {
            ...
            var percent = Math.round(1000 * d.data.Firms / total) / 10;
            tooltip2.select('.Industry').html("<strong>" + d.data[criteriaChosen] + "</strong> <span>");
            tooltip2.select('.Firms').html("Firms: " + addCommas(d.data.Firms));
            tooltip2.select('.percent').html("Proportion: " + percent + '%');
